package io.homiex.api.client.security;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean isApiKeyRequired = original.header(HomiexConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
        boolean isSignatureRequired = original.header(HomiexConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
        newRequestBuilder.removeHeader(HomiexConstants.ENDPOINT_SECURITY_TYPE_APIKEY)
                .removeHeader(HomiexConstants.ENDPOINT_SECURITY_TYPE_SIGNED);

        // Endpoint requires sending a valid API-KEY
        if (isApiKeyRequired || isSignatureRequired) {
            newRequestBuilder.addHeader(HomiexConstants.API_KEY_HEADER, apiKey);
        }
