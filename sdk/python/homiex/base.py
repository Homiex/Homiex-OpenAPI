...
from . exceptions import HomiexAPIException, HomiexRequestException
...
class Request(object):
    ...
    def __init__(self, api_key, secret, entry_point='https://api.homiex.com/openapi/', proxies=None):
    ...
    def _handle_response(self, response):
        ...
        if not str(response.status_code).startswith('2') and not response.status_code == 400:
            raise HomiexAPIException(response)
        try:
            return response.json()
        except ValueError:
            raise HomiexRequestException('Invalid Response: %s' % response.text)
